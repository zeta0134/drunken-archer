    ...
    class CommandProcesser
    {
        public string Process(string buffer)
        {
            ...
            if (command == null)
            {
                ...
                //Zeta0134 changes: default to the "lua" command if this command does not exist
                command = GameConsoleOptions.Commands.Where(c => c.Name == "lua").FirstOrDefault();
                arguments = GetArguments("lua " + buffer);
            }
